/*
 * Демонстрация политик планирования для процессов и потоков.
 * Показывает влияние привязки к CPU и планирования на выполнение.
 * Все сущности привязываются к CPU 0 для чистоты эксперимента.
 */
...
int main() {
    ...
    // Очистка (недостижимый код в данном примере)
    free(child_stack);
    return 0;
}

...
// Однако стоит уточнить, что это псевдораспараллеливание, а не настоящее, поскольку оба процесса выполняются на одном физическом процессоре

// Функция fork() в операционных системах используется для создания нового процесса..
...
int main() {
    ...
    if (pid == 0) { // Процесс-потомок
        ...
        // Ожидание родителем завершения дочернего процесса через wait(NULL); гарантирует, что родитель завершит работу только после завершения дочернего.
